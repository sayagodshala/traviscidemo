...
public class FashionFragment extends BaseFragment {
    ...
    private void getChallenges(){
        Call<APIResponse<List<Challenge>>> callback = apiService.getChallengesAsPerCategoryPagination(Constants.MB_API_KEY, user.getAuthToken(), "fashion", 1, limit, Util.getDeviceDPI(context));
    ...
    private void getChallengesPagination(int page){
        ...
        Call<APIResponse<List<Challenge>>> callback = apiService.getChallengesAsPerCategoryPagination(Constants.MB_API_KEY, user.getAuthToken(), "fashion", page, limit, Util.getDeviceDPI(context));
        callback.enqueue(new Callback<APIResponse<List<Challenge>>>() {
            @Override
            public void onResponse(Response<APIResponse<List<Challenge>>> response) {
                progressBar.setVisibility(View.GONE);
