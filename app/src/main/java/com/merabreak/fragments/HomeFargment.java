...
/**
 * Created by ETPL-002 on 22-Sep-17.
 */
...
public class HomeFargment extends BaseFragment {
    ...
    private void getChallenges() throws NullPointerException{
        Call<APIResponse<PaginationChallenges>> callback = apiService.getPages(Constants.MB_API_KEY, user.getAuthToken(), "home", 1, limit, Util.getDeviceDPI(context));
                        ...
                        if (response.body().getMeta().getStatusCode() == 401) {
                            ...
                            if (response.body().getMeta().isStatus()) {
                                if (response.body().getValues() != null) {
                                    items = response.body().getValues().getChallenges();
                                    drawChallenges();
                                } else {
    ...
    private void getChallengesPagination(int page){
        ...
        Call<APIResponse<PaginationChallenges>> callback = apiService.getPages(Constants.MB_API_KEY, user.getAuthToken(), "home", page, limit, Util.getDeviceDPI(context));
        callback.enqueue(new Callback<APIResponse<PaginationChallenges>>() {
            @Override
            public void onResponse(Response<APIResponse<PaginationChallenges>> response) {
                progressBar.setVisibility(View.GONE);
