...
import com.merabreak.adapter.HistoryProductRecyclerViewAdapter;
...
@EFragment(R.layout.user_products)
public class UserProductsFragment extends BaseFragment {
    ...
    private void getHistory() {
        hideLoader(progress);
        showLoader(progress);
        Call<APIResponse<List<Store>>> callback = apiService.getUserProductHistory(Constants.MB_API_KEY, user.getAuthToken());
                        ...
                        if (response.body().getMeta().getStatusCode() == 401) {
                            ...
                            if (response.body().getMeta().isStatus()) {
                                if (response.body().getValues() != null && response.body().getValues().size() > 0) {
                                    ...
                                    //Log.d("products response", new Gson().toJson(items));
                                    drawItems();
                                } else {
                                    setProblem(getResources().getString(R.string.user_product_frag_prob_three), "3");
                                }
                            } else {
    ...
    private void drawItems() {
        if (getActivity() != null) {
            recyclerListItemsAdapter = new HistoryProductRecyclerViewAdapter(items, getActivity());
            recyclerListItems.setAdapter(recyclerListItemsAdapter);
            recyclerListItems.setVisibility(View.VISIBLE);
            ((HistoryProductRecyclerViewAdapter) recyclerListItemsAdapter).setOnItemClickListener(new HistoryProductRecyclerViewAdapter.MyClickListener() {
                @Override
