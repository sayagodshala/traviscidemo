...
import com.merabreak.adapter.HistoryRaffleRecyclerViewAdapter;
...
@EFragment(R.layout.user_raffles)
public class UserRafflesFragment extends BaseFragment {
    ...
    private void getHistory() {
        hideLoader(progress);
        showLoader(progress);
        Call<APIResponse<List<Raffle>>> callback = apiService.getUserRaffleHistory(Constants.MB_API_KEY, user.getAuthToken());
                        ...
                        if (response.body().getMeta().getStatusCode() == 401) {
                            ...
                            if (response.body().getMeta().isStatus()) {
                                if (response.body().getValues() != null && response.body().getValues().size() > 0) {
                                    ...
                                    //Log.d("raffles response", new Gson().toJson(items));
                                    drawItems();
                                } else {
                                    setProblem(getResources().getString(R.string.user_raffle_frag_prob_three), "3");
                                }
                            } else {
    ...
    private void drawItems() {
        if (getActivity() != null) {
            recyclerListItemsAdapter = new HistoryRaffleRecyclerViewAdapter(items, getActivity());
            recyclerListItems.setAdapter(recyclerListItemsAdapter);
            recyclerListItems.setVisibility(View.VISIBLE);
            ((HistoryRaffleRecyclerViewAdapter) recyclerListItemsAdapter).setOnItemClickListener(new HistoryRaffleRecyclerViewAdapter.MyClickListener() {
                @Override
