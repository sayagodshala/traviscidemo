...
public class PielView extends View {
    ...
    private List<SpinWheelIndex> mLuckyItemList;
    ...
    private void init() {
        ...
        strokePaint.setTextSize(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 16,
                getResources().getDisplayMetrics()));
    ...
    public void setData(List<SpinWheelIndex> luckyItemList) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mLuckyItemList.size(); i++) {
            ...
            drawText(canvas, tmpAngle, sweepAngle, mLuckyItemList.get(i).getCoins());
            drawTextPoints(canvas, tmpAngle, sweepAngle, mLuckyItemList.get(i).getText());
    ...
    private void drawCenterImage(Canvas canvas, Drawable drawable) {
        ...
        Bitmap bitmap = WheelViewUtils.drawableToBitmap(drawable);
    ...
    private void drawText(Canvas canvas, float tmpAngle, float sweepAngle, String mStr) {
        ...
        int vOffset = mRadius / 7;

        canvas.drawTextOnPath(mStr, path, hOffset, vOffset, mTextPaint);
        canvas.drawTextOnPath(mStr, path, hOffset, vOffset, strokePaint);
    }

    private void drawTextPoints(Canvas canvas, float tmpAngle, float sweepAngle, String mStr) {
        Path path = new Path();
        path.addArc(mRange, tmpAngle, sweepAngle);
    ...
    public void rotateTo(int index) {
       ...
       // float targetAngle = 360 * mRoundOfNumber + 270 - getAngleOfIndexTarget() + (360 / mLuckyItemList.size()) / 2;
        float targetAngle = 360 * mRoundOfNumber + 360 - getAngleOfIndexTarget() + (360 / mLuckyItemList.size()) / 2;
        animate()
